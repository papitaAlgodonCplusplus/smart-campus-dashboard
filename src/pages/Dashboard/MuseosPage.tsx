...
const MuseosPage: React.FC = () => {
  const { loading, error, categorizedSpaces } = useDashboard();
  const categoryName = 'Museos y Reservas';
  const museosSpaces = categorizedSpaces.museos;
  const [view3D, setView3D] = useState(false);
  
  // Calculate stats for this category
  const totalCapacity = museosSpaces.reduce((acc, space) => acc + space.capacity, 0);
  const totalOccupancy = museosSpaces.reduce((acc, space) => acc + space.currentOccupancy, 0);
            ...
            {view3D ? (
              <Box sx={{
                ...
                <Typography variant="subtitle1" gutterBottom>
                  Galería 3D de Museos - Haz clic en cualquier marco para ampliar
                </Typography>
                <br />
                <br />
                <Gallery spaces={museosSpaces} />
              </Box>
            ) : (
              <CategorySpaces
                categoryName={categoryName}
